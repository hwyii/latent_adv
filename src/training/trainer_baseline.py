...
def build_model(cfg, num_labels=2):
    ...
    dtype_str = cfg["model"].get("torch_dtype", None)
    torch_dtype = {"bfloat16": torch.bfloat16, "float16": torch.float16}.get(dtype_str, None)
...
def build_opt_sched(model, cfg, train_loader):
    ...
    total_steps = (len(train_loader) // accum) * int(cfg["train"]["epochs"])
    warmup_steps = 0
...
def train_one_epoch(model, loader, device, optim, sched, scaler=None, log_every=50,
    ...
    for step, batch in enumerate(loader, 1):
        ...
        running_loss += loss.item() * accum_steps  # log unscaled loss
        if step % log_every == 0:
...
def train_baseline(cfg: dict):
    ...
    for epoch in range(1, cfg["train"]["epochs"] + 1):
        ...
        train_one_epoch(model, train_loader, device, optim, sched, scaler,
