...
#from ..utils.tools import get_outputs_from_reft
...
def load_model_from_state(model_name, ckpt_path, device):
    print(f"[DEBUG] loading ckpt from: {ckpt_path}")
    state = torch.load(ckpt_path, map_location=device)
    print(f"[DEBUG] ckpt type = {type(state)}")
    if isinstance(state, dict):
        print(f"[DEBUG] ckpt top-level keys: {list(state.keys())[:292]}")

    model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=2)

    try:
        missing, unexpected = model.load_state_dict(
            state if not isinstance(state, dict) or "model" not in state else state["model"],
            strict=False  # 先直接 strict=False 并拿到 missing / unexpected
        )
        print(f"[DEBUG] missing keys: {len(missing)}, unexpected keys: {len(unexpected)}")
        # 如果 missing 非常多, unexpected 也很多, 那基本就是没对上
    except Exception as e:
        print(f"[DEBUG] load_state_dict failed with error: {e}")
        raise
...
def run_evaluations(
    ...
    adv = load_model_from_state(model_name, adv_ckpt, device) # 看这个adv是什么
    # models = {"baseline": baseline, "adv": adv}
    print(f"[DEBUG] model fingerprint: {model_fingerprint(adv):.6e}")
    models = {"adv": adv}
    ...
    print(">>> [Timer] 开始 GPU 预热 (运行 1 个样本)...")
    try:
        for i in range(min(1, n_samples)): # 预热 1 个样本
            item = ds[i]
